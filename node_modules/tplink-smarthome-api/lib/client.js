...
const smart_discovery_1 = require("./smart-discovery");
...
class Client extends events_1.EventEmitter {
    ...
    async getSmartSysInfo(host, port, sendOptions, transport) {
        ...
        if (responseObj.error_code !== 0) {
            throw new smart_error_1.default('SMART request failed', responseObj.error_code, request.method, JSON.stringify({ error_code: responseObj.error_code }), JSON.stringify(request));
        }
        if (!(0, utils_1.isObjectLike)(responseObj.result)) {
            throw new Error('Unexpected SMART get_device_info response');
        }
        const sysInfo = (0, smart_discovery_1.normalizeSmartSysInfo)(responseObj.result, {
            transport,
            port: port ?? 80,
        });
    ...
    static inferDeviceTransportDefaultsFromSysInfo(sysInfo, deviceOptions) {
        ...
        if (resolvedDefaultSendOptions.protocol === undefined &&
    ...
    startDiscovery(options = {}) {
        ...
        try {
            ...
            this.socket = socket;
            const processDiscoveredSysInfo = (sysInfo, rinfo, useDiscoveryPort) => {
            ...
            socket.on('message', (msg, rinfo) => {
                if (smart_discovery_1.SMART_DISCOVERY_PORTS.some((smartDiscoveryPort) => smartDiscoveryPort === rinfo.port)) {
                    try {
                        const packet = (0, smart_discovery_1.parseSmartDiscoveryPacket)(msg);
                        const response = packet.payload;
                        if (!isSmartResponse(response)) {
                            throw new Error('Unexpected SMART discovery response');
                        }
                        if (response.error_code !== 0) {
                            throw new smart_error_1.default('SMART discovery request failed', response.error_code, 'discovery', JSON.stringify({ error_code: response.error_code }), 'TDP v2 discovery probe');
                        }
                        if (!(0, utils_1.isObjectLike)(response.result)) {
                            throw new Error('Unexpected SMART discovery result');
                        }
                        const sysInfo = (0, smart_discovery_1.normalizeSmartSysInfo)(response.result);
                        if (!(0, device_1.isPlugSysinfo)(sysInfo)) {
                            throw new Error('Unexpected SMART discovery device info');
                        }
                        this.log.debug('client.startDiscovery(): received SMART TDP response from %s:%s', rinfo.address, rinfo.port);
                        ...
                        this.log.debug('client.startDiscovery(): invalid SMART TDP response from %s:%s: %s', rinfo.address, rinfo.port, err);
    ...
    sendDiscovery(socket, address, devices, offlineTolerance) {
        ...
        try {
            ...
            const smartDiscoveryMsg = (0, smart_discovery_1.createSmartDiscoveryProbe)();
            smart_discovery_1.SMART_DISCOVERY_PORTS.forEach((smartDiscoveryPort) => {
                socket.send(smartDiscoveryMsg, 0, smartDiscoveryMsg.length, smartDiscoveryPort, address);
            });
            devices.forEach((d) => {
                ...
                smart_discovery_1.SMART_DISCOVERY_PORTS.forEach((smartDiscoveryPort) => {
                    socket.send(smartDiscoveryMsg, 0, smartDiscoveryMsg.length, smartDiscoveryPort, d.host);
                });
            });
