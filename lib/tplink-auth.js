...
function getEp10ClientOptions(deviceData = {}, settings = {}, globalCredentials = null) {
  ...
  return {
    defaultSendOptions: { transport,
      ...(['iot', 'smart'].includes(deviceData.protocol) ? { protocol: deviceData.protocol } : {}),
    },
    ...(transport === 'tcp'
      ? {}
...
function getDeviceConnectionData(device) {
  const data = device.getData();
  const saved = typeof device.getStoreValue === 'function' && device.getStoreValue('tplinkConnection');
  return saved && saved.host === device.getSettings().settingIPAddress && isValidTpLinkTransport(saved.transport)
    ? { ...data, transport: saved.transport, protocol: saved.protocol }
    : data;
}
...
async function getKs240SysInfo(client, host) {
  try {
    return await client.getSysInfo(host);
  } catch (error) {
    // Some KS240 firmware advertises KLAP and rejects the AES
    // handshake before any credentials are sent. Do not retry login failures.
    if (!/^AesConnection\([^\n]*\): handshake failed with error_code 1003$/.test(error && error.message || '')) {
      throw error;
    }
    return client.getSysInfo(host, undefined, { transport: 'klap' });
  }
}
