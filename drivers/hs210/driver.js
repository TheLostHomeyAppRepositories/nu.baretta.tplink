...
class TPlinkPlugDriver extends Homey.Driver {
  async onPair(session) {
    ...
    const discoverHs210Devices = (input, targetHost) => {
      stopActiveDiscovery();
        ...
        const validatePlug = async plug => {
          try {
            const transport = isValidTpLinkTransport(
              ...
              : 'tcp';
            this.log(`HS210 discovery: ${plug.host}, transport=${transport}, account=${credentialResolution ? credentialResolution.source : 'none'}`);
    ...
    session.setHandler('discover', async data => {
      ...
      const result = await discoverHs210Devices(input);
    ...
    session.setHandler('get_devices', async data => {
      ...
      for (const input of inputs) {
        if (isIP(input.ip) !== 4) {
          throw new Error('A valid IPv4 address is required to pair an HS210 manually.');
        }

        const result = await discoverHs210Devices(input, input.ip);
        if (!pairingOpen || version !== requestVersion) return [];
        ...
        if (!target) {
          ...
          if (result.authenticationRequired) {
            throw new Error(
              'This HS210 uses authenticated firmware. Enter complete TP-Link account credentials and try again.',
            );
          }
          throw new Error(
            'No accessible HS210 was found at the supplied IP address. Check the address and try discovery again.',
          );
        }
        if (input.deviceId && input.deviceId !== target.deviceId) {
          throw new Error(
            'The HS210 at the supplied address no longer matches the device selected during discovery.',
          );
        }
        if (knownDeviceIds.has(target.deviceId)) {
          throw new Error('This HS210 is already paired.');
        }
