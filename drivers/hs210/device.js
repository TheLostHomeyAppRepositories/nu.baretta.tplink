...
class TPlinkPlugDevice extends Homey.Device {
    ...
    async onInit() {
        ...
        this.log('HS210 transport configured: ' + this.activeTransport);
        ...
        try {
            ...
            if (credentialsChanged) {
                ...
                if (!transition.credentials && data.transport !== 'tcp') {
                    throw new Error(
                        'Complete global TP-Link account credentials are required before clearing credentials for authenticated HS210 firmware.'
                    );
                }
