...
test('HS210 uses the saved account, filters other models, validates once and reports rejected authentication', async () => {
  let calls = 0;
  let rejected = false;
  const account = { username: 'owner@example.com', password: 'private-password' };
  const plug = {
    host: '192.168.10.101', model: 'HS210(US)', deviceId: 'discovery-hash',
  ...
  const Driver = loadFreshModule('../drivers/hs210/driver.js', {
...
test('HS210 save revalidates a selected device with the exact credentials to persist', { concurrency: false }, async () => {
  ...
  await withFakeTimers(async timers => {
    ...
    timers[0].callback();
    sysInfo.resolve({ model: 'HS210', deviceId: 'selected-device-id', alias: 'Physical HS210' });
...
test('HS210 save rejects a selected device when targeted validation finds a different device ID', { concurrency: false }, async () => {
  const plug = {
    host: '192.0.2.11',
    model: 'HS210',
    deviceId: 'physical-device-id',
    defaultSendOptions: { transport: 'klap' },
    getSysInfo: async () => ({ model: 'HS210', deviceId: 'physical-device-id' }),
  };
  const { Client, instances } = createClientStub(client => client.emit('plug-new', plug));
  const Driver = loadFreshModule('../drivers/hs210/driver.js', {
    homey: createHomeyStub(),
...
test('HS210 TCP pairing does not persist or seed unused account credentials', { concurrency: false }, async () => {
  const plug = {
    host: '192.0.2.13',
    model: 'HS210',
    deviceId: 'tcp-device-id',
    defaultSendOptions: { transport: 'tcp' },
    getSysInfo: async () => ({ model: 'HS210', deviceId: 'tcp-device-id' }),
  };
  const { Client, instances } = createClientStub(client => client.emit('plug-new', plug));
  const Driver = loadFreshModule('../drivers/hs210/driver.js', {
    homey: createHomeyStub(),
...
test('HS210 authenticated discovery requires a complete account pair before returning a device', { concurrency: false }, async () => {
  let sysInfoCalls = 0;
  const plug = {
    host: '192.0.2.14',
    model: 'HS210',
    ...
    getSysInfo: async () => {
      sysInfoCalls += 1;
      return { model: 'HS210', deviceId: 'authenticated-device-id' };
    },
  };
...
test('HS210 rediscovery keeps the existing client and transport when the IP settings update fails', { concurrency: false }, async () => {
  ...
  const plug = {
    ...
    deviceId: 'device-id',
    defaultSendOptions: { transport: 'klap' },
    getSysInfo: async () => ({ model: 'HS210', deviceId: 'device-id' }),
  };
  const { Client, instances } = createClientStub(client => client.emit('plug-new', plug));
  const Device = loadFreshModule('../drivers/hs210/device.js', {
    homey: createHomeyStub(),
    'tplink-smarthome-api': { Client },
  });
