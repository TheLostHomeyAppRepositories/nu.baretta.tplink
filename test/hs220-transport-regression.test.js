...
test('HS220 uses the saved account, filters other models, validates once and reports rejected authentication', async () => {
  let calls = 0;
  let rejected = false;
  const account = { username: 'owner@example.com', password: 'private-password' };
  const plug = {
    host: '192.168.10.101', model: 'HS220(US)', deviceId: 'discovery-hash',
  ...
  const Driver = loadFreshModule('../drivers/hs220/driver.js', {
...
test('HS220 save revalidates a selected device with the exact credentials to persist', { concurrency: false }, async () => {
  ...
  await withFakeTimers(async timers => {
    ...
    timers[0].callback();
    sysInfo.resolve({ model: 'HS220', deviceId: 'selected-device-id', alias: 'Physical HS220' });
...
test('HS220 save rejects a selected device when targeted validation finds a different device ID', { concurrency: false }, async () => {
  const plug = {
    host: '192.0.2.11',
    model: 'HS220',
    deviceId: 'physical-device-id',
    defaultSendOptions: { transport: 'klap' },
    getSysInfo: async () => ({ model: 'HS220', deviceId: 'physical-device-id' }),
  };
  const { Client, instances } = createClientStub(client => client.emit('plug-new', plug));
  const Driver = loadFreshModule('../drivers/hs220/driver.js', {
    homey: createHomeyStub(),
...
test('HS220 TCP pairing does not persist or seed unused account credentials', { concurrency: false }, async () => {
  const plug = {
    host: '192.0.2.13',
    model: 'HS220',
    deviceId: 'tcp-device-id',
    defaultSendOptions: { transport: 'tcp' },
    getSysInfo: async () => ({ model: 'HS220', deviceId: 'tcp-device-id' }),
  };
  const { Client, instances } = createClientStub(client => client.emit('plug-new', plug));
  const Driver = loadFreshModule('../drivers/hs220/driver.js', {
    homey: createHomeyStub(),
...
test('HS220 authenticated discovery requires a complete account pair before returning a device', { concurrency: false }, async () => {
  let sysInfoCalls = 0;
  const plug = {
    host: '192.0.2.14',
    model: 'HS220',
    ...
    getSysInfo: async () => {
      sysInfoCalls += 1;
      return { model: 'HS220', deviceId: 'authenticated-device-id' };
    },
  };
...
test('HS220 rediscovery keeps the existing client and transport when the IP settings update fails', { concurrency: false }, async () => {
  ...
  const plug = {
    ...
    deviceId: 'device-id',
    defaultSendOptions: { transport: 'klap' },
    getSysInfo: async () => ({ model: 'HS220', deviceId: 'device-id' }),
  };
  const { Client, instances } = createClientStub(client => client.emit('plug-new', plug));
  const Device = loadFreshModule('../drivers/hs220/device.js', {
    homey: createHomeyStub(),
    'tplink-smarthome-api': { Client },
  });
